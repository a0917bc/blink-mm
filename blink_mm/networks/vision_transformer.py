...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # self.attn_score = torch.compile(AttnScore(scale=self.scale, attn_drop=attn_drop))
...
class VisionTransformer(nn.Module):
    ...
    def forward_head(self, x, pre_logits: bool = False):
        ...
        x = self.fc_norm(x)
        x = self.head_drop(x)
        return x if pre_logits else self.head(x) # self.head = nn.Linear(self.embed_dim, num_classes) if num_classes > 0 else nn.Identity()
...
#     'vit_small_patch16_224.dino': _cfg( # deit3_small_patch16_224.fb_in22k_ft_in1k
